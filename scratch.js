...
// .filter in this example is covering returning the blogs in the array that have "amet" in their category
const filteredBlogs = sampleBlogs.filter((sampleBlog) => {
    // Version 1
    const isInAmetCat = sampleBlog.category.includes("amet");
    ...
    return false;
    // Version 2
    // if (isInAmetCat === false) {
    // 	return true
    // }
    // return false;

    // Version 3
    // return sampleBlog.category.includes("amet")
});
...
// .map is a specified sort of loop. In this case it is going one item at a time (a lot like for of look) from filteredBlogs and adding " " + "whateverDUDE" to every author within filteredBlogs
const mappedBlogs = filteredBlogs.map((sampleBlog) => {
    const mappedBlog = {
        title: sampleBlog.title,
        text: sampleBlog.text,
        author: sampleBlog.author + " " + "whateverDUDE",
        categories: sampleBlog.category,
        createdAt: sampleBlog.createdAt,
        lastModified: sampleBlog.lastModified,
    };
    return mappedBlog;
});
...
// code below is doing what the other two functions above are doing but together in one. 
const mappedAndFilteredBlogs = sampleBlogs
    .filter((sampleBlog) => {
        const isInAmetCat = sampleBlog.category.includes("amet");
